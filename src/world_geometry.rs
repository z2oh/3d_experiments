...
impl WorldGeometryManager {
    ...
    /*
    pub fn add_chunk(&mut self, x: i32, y: i32) {
        // Generate our transform matrix for this chunk.
        let t = Matrix4::from_translation(Vector3::new(
                (x * self.chunk_dim as i32) as f32,
                (y * self.chunk_dim as i32) as f32,
                0.0,
        ));

        // Now generate the actual mesh for the chunk.
        let (vertices_n, indices_n) = generate_chunk_x_y(x, y, self.chunk_dim, &self.noise).report();
        let chunk_index = ChunkIndex {
            vertex_offset: self.vertex_buf.len(),
            index_offset: self.index_buf.len(),
            index_count: self.indices_n.len(),
            transform_index: self.chunk_transforms.len(),
        };

        // And update our local accumulators.
        self.index_buf.extend(vertices_n);
        self.indices.extend(indices_n);
        self.chunk_transforms.push(t.into());
